    ...
    function filesystem:Ls(dir)
        ...
        if hfile ~= INVALID_HANDLE then
            repeat
                ...
                if fd.dwFileWttributes == 0x10 or fd.dwFileWttributes == 0x11 or (self.show_hidden and fd.dwFileWttributes == 0x2012) then
        ...
        if not self.show_hidden then removevalue(tfiles, 'hidden') end
        table.sort(tfiles)
